...
\begin{document}
    ...
    eigenstates $R_{n,l,s,j}$ is formulated.

    The first step is to express the eigenvalue problems in terms of
    dimensionless quantities. To this end the Hamiltonians $H_{l,s,j}$ in
    Eq.~\eqref{eq:BlockHamiltoniansSIUnits} are expressed in units of Hartree
    \cite{friedrich2017},
    ...
    Bohr's radius. The radial eigenfunctions are parametrised as
    ...
    eigenfunctions \eqref{eq:DimensionlessEigenfunctions} take the form
    ...
    the basis functions, are constructed and subsequently the matrix
    ...
    the interval $I$. Strictly speaking the support of the radial eigenstates is
    not finite, as they typically approach zero only asymptotically. However, in
    practice the support can be considered finite whenever the value of the
    ...
    $d\geq1$ is the degree of the B-splines: each B-spline is a piecewise
    ...
    are chosen in strictly ascending order. This construction can be visualised
    as follows:
    \begin{equation}
    ...
    $k=d+1$ times, where $k$ is called the \emph{order} of the B-splines.

    On the knots $(t_i)_{i=0,\dots,N-1+2d}$ the $N_\mathrm{bs}=N+d-1$ many
    ...
    $[0,1]$.\footnote{The B-splines are defined on the full set $\mathbb{R}$
    with the understanding that each B-spline is zero outside the interval $I$.}
    Further one shows that
    \begin{equation}
    ...
    uniquely defined for the choice of knots $(t_i)_{i=0,\dots,N-1+2d}$. With
    this the construction of the B-spline basis is constructed. The next step is
    to express the eigenvalue problem posed by
    Eq.~\eqref{eq:DimensionlessEigenproblem} in this basis.
    ...
    \,.
    \end{equation}
    To simplify the notation the indices on $f_{n,l,s,j}$ were dropped and the
    dimensionless energy renamed to $\lambda=\bar{E}_{n,l,s,j}$. Further, the
    potential terms in Eq.~\eqref{eq:DimensionlessEigenproblem} are collected
    into the function
    ...
    \,.
    \end{equation}
    Here, $\bm{f}=(f_1,\dots,f_{N_\mathrm{bs}-1})^\mathrm{T}$ is the vector
    containing the expansion coefficients $f_i$. It is important to note that
    this is an approximation, because in general one must employ an infinite
    basis to exactly represent the eigenfunctions $f$. A brief discussion
    regarding this approximation is reserved for later in this section. For now,
    the expansion of the eigenfunctions $f$ given in
    ...
    \begin{equation}\label{eq:IntermediateStepIWeakEigenvalueProblem}
    ...
    Next, the homogeneous Dirichlet boundary conditions
    $f(0)=f(t_\mathrm{max})=0$ are enforced on $f$ in
    Eq.~\eqref{eq:ExpansionOfEigenfunctions}. To this end one notices that
    for the choice of knots employed in this work the de Boor algorithm
    ...
    this result the sums in
    Eq.~\eqref{eq:IntermediateStepIWeakEigenvalueProblem} is be reduced by the
    boundary terms:
    \begin{equation}\label{eq:IntermediateStepIIWeakEigenvalueProblem}
    ...
    for all indices $i,j$ the B-splines obey
    ...
    posed by Eq.~\eqref{eq:IntermediateStepIIWeakEigenvalueProblem} takes the
    form
    \begin{equation}\label{eq:MatrixEigenvalueProblem}
        (K + W) \bar{\bm{f}} = \lambda M \bar{\bm{f}}
    \,,
    \end{equation}
    ...
    vector with respect to the scalar product $B$ represented by the
    positive-definite matrix $M$, i.e., in order to normalise $f$ one must
    normalise $\bar{\bm{f}}$ such that
    \begin{equation}
    ...
    \texttt{DSBGVX} from the software collection \texttt{LAPACK} \cite{laug1999}
    taken from \emph{Netlib} \cite{dongarra1987}. This routine is specifically
    designed to solve real-symmetric banded generalised eigenvalue problems. It
    further allows to extract eigenvalues selectively. This is useful when the
    dimension $N_\mathrm{bs}-2$ of the eigenvalue problem is much larger than the
    number of eigenstates that shall be computed.
    ...
    eigenvalue problem posed by Eq.~\eqref{eq:RadialEigenproblem} are given by
    the (normalised) stationary points of the following functional:
    ...
    eigenvalue and the exact radial eigenstate as the number of knots, i.e.,
    $N$, is increased. Therefore, one can be sure that the solutions found by
    solving the generalised matrix eigenvalue problem indeed approximate the
    exact radial eigenenergies $E_{n,l,s,j}$ and eigenstates $R_{n,l,s,j}$,
    without producing solutions that do not converge to solutions of the exact
    eigenvalue problem for the radial eigenstates $R_{n,l,s,j}$.
    ...
    knots $t_i$ are spaced sufficiently close, on each interval $[t_i,t_{i+1})$
    ...
    \,.
    \end{equation}
    Here $\bar{\bm{f}}_{n,l,s,j}$ denotes the coefficient vector
    $\bm{f}_{n,l,s,j}$ stripped of the first and final component. Further, the
