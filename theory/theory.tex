...
\begin{document}
    ...
    optics, \dots. However, expect for the special case of the Hydrogen atom and
    ...
    library functions. This documents purpose is to add the missing pieces: in
    ...
    \alkcalc employs the software packet \texttt{GAUSSQ} \cite{golub1969} from
    ...
    quadrature order $n=2(d-1)$, because the derivative of a B-spline is a
    piecewise polynomial of degree $d-1$. Of course, $n=2(d-1)$ is only sensible
    for $d\geq2$, because otherwise the second derivative vanishes and therefore
    all components of $K$. In practice, this is not a restriction because
    typical B-spline orders $(k=d+1)$ are at least $k{\sim}6$ or higher. To
    compute the components of the matrix $M$ one chooses the quadrature order
    $n=d+1$. With this choice polynomials of degree $2(d+1)-1=2d+1$ are
    integrated exactly, which is enough, considering that the relevant integrals
    are over polynomials of degree $2d$. Finally, the components of the matrix
    $W$ representing the effective single-electron potential must be calculated.
