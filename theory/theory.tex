...
\begin{document}
    ...
    \end{equation}
    {\color{blue}
    Finally, to solve the generalised eigenvalue problem posed by
    Eq.~\eqref{eq:MatrixEigenvalueProblem} \alkcalc employs the routine
    ??? from the software collection LAPACK \cite{LAPACK}.

    At this point there are two more open questions to answer. \textbf{(i)} The
    ...
    approximate the exact result. For instance, on glimpse one might think that
    it is possible to find eigenvalues which are not physical but solely
    ...
    \textbf{(i)} Ritz-variational methods for argumentation

    \textbf{(ii)} Gaussian quadratures

    \newpage

    % - Talk about approximation and Ritz variational method
    % - Introduce Gaussian quadratures
    ...
    \begin{align}\label{eq:radialMatrixElement}
        ...
        \end{split}
    \end{align}
    Here, `$\mathrm{id}$' denotes the identity map. The remaining scalar product
    can be approximated with solutions of the matrix eigenproblem in
    Eq.~\eqref{eq:MatrixEigenproblem}, i.e., $f_{n,l,s,j}$ and $f_{n',l',s',j'}$
    are represented by the vectors $\bm{f}_{n,l,s,j}$ and
    $\bm{f}_{n',l',s',j'}$, respectively [see Eq.~\eqref{eq:ExpansionOfState}].
    Within this approximations
    \begin{equation}
        B(f_{n,l,s,j}, \mathrm{id}^p f_{n',l',s',j'})
        =
