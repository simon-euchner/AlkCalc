...
\begin{document}
    ...
    {\color{blue}
    The ultimate goad is to express Eq.~\ref{fig:DimensionlessEigenproblem} as a
    matrix eigenvalue problem which can be directly addressed with a computer.
    ...
    different basis states to choose from. In \alkcalc the so-called
    \emph{B-splines} are chosen.\footnote{This method may be conceptually and
    programmatically somewhat more involved compared to a standard
    finite-difference or finite-element method, however, it is preferred here
    for two reasons: B-splines allow to work with much smaller matrix
    dimensions and therefore allow to store the resulting eigenenergies and
    radial eigenstates much more efficiently. Additionally, the matrices will
    be symmetric and much smaller in dimension compared } B-splines are piecewise polynomials with finite support. In the following it is discussed how the
    B-splines (basis functions) are constructed and subsequently the matrix
    eigenproblem is formulated.

    The starting point is to truncate the domain of the radial eigenfunctions
    $f_{n,l,s,j}$ from $\mathbb{R}_{\geq0}$ to a finite interval
    $I=[0,t_\mathrm{max}]\subset\mathbb{R}_{\geq0}$, where $t_\mathrm{max}$
    defined the maximal radius $r_\mathrm{max}=\aB{t}_\mathrm{max}$. This
    truncation is justified whenever the maximal $t_\mathrm{max}$ is chosen
    sufficiently large such that the support of the radial eigenfunctions
    $f_{n,l,s,j}$ of interest lays within the interval $I$. Strictly speaking
    the support of the radial eigenstates is not finite, as they typically
    approach zero only asymptotically. However, in practice the support can be
    considered finite whenever the value of the radial eigenstates stays below
    machine precision. To construct the B-splines one chooses a family of points
    $(t_i)_{i=0,\dots,N-1+2d}$ in the interval $I$, the so-called
    \emph{knots}.\footnote{The family $(t_i)_{i=0,\dots,N-1+2d}$ is sometimes
    called the knot vector in the literature.} Here, $d\geq0$ is the degree of the
    degree of the B-splines, i.e., the degree of the polynomials that
    constitute the basis functions. The knots $t_i$ are defined as
    \begin{equation}
        t_i
        =
        \begin{cases}
    ...
    \,,
    \end{equation}
    where the knots are assumed to be strictly ascending:
    $t_{k-1}<\dots<t_{k+N-2}$. Here, $N$ is the number of distinct
    discretisation points and the $N-2$ knots $t_{k,\dots,k+N-3}$ are the
    so-called \emph{internal knots}. This construction can be visualised as
    follows:
    \begin{equation}
    ...
    \,.
    \end{equation}
    On the set of knots, the B-splines are defined via de Boor's recursion
    formula:
    \begin{equation}
        u^{(r)}_i(t)
        =
        \frac{t-t_i}{t_{i+r}-t_i}
        u^{(r-1)}_i(t)
        +
        \frac{t_{i+r-1}-t}{t_{i+r-1}-t_{i+1}}
        u^{(r-1)}_{i+1}(t)
    \,, \ \ \
        u^{(0)}_i(t)
        =
        \begin{cases}
            1
            &, \hspace*{10mm} t_i \leq t \leq t_{i+k}
        \\
            0
    ...
    \,.
    \end{equation}
    In conjugation with the normalisation condition this yields a result that is
    unique. To evaluate the B-splines at some point numerically de Boor's
    algorithm is employed \cite{}, rewritten by E. Amos for double precision
    floating point arithmetic. The final basis is denoted as
    \begin{equation}
        (u^{(d)}_i)_{i=0,\dots,N_\mathrm{bs}-1}
    \,, \ \ \
        N_\mathrm{bs} = k + N - 1
    \,, \ \ \
        u^{(d)}_i\colon[t_i,t_{i+k}) \to \mathbb{R}
    \,,
    \end{equation}
    where $N_\mathrm{bs}$ is the total number of B-splines.

    At this point the B-splines, i.e., the basis functions, are constructed. The
    next step is to express the eigenvalue problem posed by
    Eq.~\eqref{eq:DimensionlessEigenproblem} in this basis.

    % Notes
    %To understand why each B-spline must spane an aread of k knots, note that
    %otherwise I canot bring it doen to zero again for a degree k Polynomial
    %The multiplicity on the boundary of the knots will be useful for ...

    To obtain a matrix problem which approximates
    Eq.~\eqref{eq:eigenproblem} one computes projections of it
