...
\begin{document}
    ...
    alkaline-earth metal ions are of relevance in quantum computing, quantum
    information processing, quantum simulation, Rydberg physics, quantum optics,
    \dots. However, expect for Hydrogen, it is not possible to solve the
    time-independent Schr\"odinger equation analytically. Because of this, there
    is the need to obtain eigenenergies, i.e., atomic spectra,  and eigenstates
    numerically. The library \alkcalc is intended for exactly this purpose. In
    the various \texttt{README} files included in \alkcalc technical details for
    running the software are provided. However, in these \texttt{README} files,
    there is no theoretical discussion on the theory behind the code. This is
    the purpose of the present document.

    In this document the class of Hamiltonians which \alkcalc is designed to
    diagonalise is presented. Moreover, this document formally defines each
    quantity which \alkcalc is designed to compute. Therefore, this document
    serves as a reference for the user. In particular, the final section is a
    complete reference manual for the library functions of \alkcalc.

    %%% SECTION
    \section{Theory}
    \label{sec:Theory}

    We start by considering a single atom/ion. Subsequently, we describe the
    \ac{fem} used to numerically solve the Schr\"odinger equation. Finally, we
    describe how \alkcalc computes radial matrix elements, oscillator strengths,
    and state lifetimes.
    ...
    nucleus' size and $\alpha_\mathrm{d}$ its polarisability. Finally,
    ...
    $\mathrm{SU}(2)$ are $(2j+1)$-dimensional, with
    $j=0,1\slash{2},1,3\slash{2},2,\dots$, such that the blocks have dimensions
    $\mu_j(2j+1)$, where $\mu_j$ is the multiplicity of the irreducible
    ...
    all final $m'_j$, because we are interested in transitions between the
    ...
    \,, \
        ...
        \sum_{u=x,y,z}
        \abs*{\langle\bm{\Psi}_\mathrm{f},r_u\slash\aB\bm{\Psi}_\mathrm{i}\rangle}^2
    \,.
    \end{equation}
    ...
    \begin{equation}\label{eq:SpontaneousRate}
        ...
        \sum_{n',l',j'}
        E^2_\mathrm{i,f} (-f_{\mathrm{i}\to\mathrm{f}})
    \,.
    \end{equation}
    The rate in Eq.~\eqref{eq:SpontaneousRate} is the spontaneous emission rate,
    which only appears for transitions where $E_\mathrm{i}>E_\mathrm{f}$.  Note
    that, implicitly, this restricts the quantum numbers the sum is running
    over. Further, this is the correct decay rate only for zero temperature
    case. At finite temperature, spontaneous transitions increasing the energy
    may be driven as well, if thermal fluctuations are sufficiently strong. The
    background spectral density is given by Planck's law, which yields for the
    average photon number at frequency $\omega$ and temperature $T$:
    \begin{equation}
        n(\omega,T)
        =
