...
\begin{document}
    ...
    is to choose a family of points $(t_i)_{i=0,\dots,N-1+2d}$ in the interval
    $I$, the so-called \emph{knots}.\footnote{The family
    $(t_i)_{i=0,\dots,N-1+2d}$ is sometimes called the \emph{knot vector} in the
    literature.} Here, $N\in\mathbb{N}$ is the number of unique knots and
    $d\geq0$ is the degree of the B-splines: each B-spline is a piecewise
    polynomial function; a degree-$d$ B-spline is a B-spline where all these
    polynomials have (maximally) degree $d$. The knots $t_i$ are defined as
    \begin{equation}
        t_i
        =
        \begin{cases}
