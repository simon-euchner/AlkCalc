...
\begin{document}
    ...
    $t_\mathrm{max}$ defines the maximally considered radius
    ...
    \,, \ \ \
        ...
        \begin{cases}
            1
            &, \hspace*{10mm} t_i \leq t \leq t_{i+1}
        \\
            0
    ...
    \,.
    \end{equation}
    The support of the resulting B-splines is restricted to the finite interval
    $[t_i,t_{i+k})$, i.e., the B-splines can be interpreted as real functions
    $u^{(d)}_i\colon[t_i,t_{i+k})\to\mathbb{R}$. One can show that with the
    chosen initial condition in Eq.~\eqref{eq:DeBoorsRecursion} all B-splines
    $u^{(d)}_i\colon[t_i,t_{i+k})\to[0,1]$, i.e., map into the interval [0,1].
    ...
    $\alpha,\beta\colon{I}\to\mathbb{R}$. To finally bring the eigenproblem into
    matrix form the following matrices are defined: first, the so-called
    \emph{stiffness} matrix $K$ with components
    \begin{equation}
        K_{ij}
        =
    \,.
    \end{equation}
    Note that the matrices $K$, $M$, and $W$ are real, symmetric, and banded.
    These properties later allow to employ efficient specialised numerical
    methods to solve for the coefficient vectors $\bm{f}$ and the eigenvalues
    $\lambda$.
    ...
    \,.
    \end{equation}
    The Dirichlet boundary conditions are $f(0)=f(t_\mathrm{max})=0$. To enforce
    these, one sets $c_0=c_{N_\mathrm{bs}-1}=0$. In this case, one can write the
    function as
    \begin{equation}
        f(t)
        =
    ...
    \,.
    \end{equation}
    This shows that any radial eigenfunction $f$ is represented by
    $N_\mathrm{bs}-2$ many coefficients, which in the following are collected in
    the vector $\bm{f}^\mathrm{T}=[f_1,\dots,f_{N_\mathrm{bs}-1}]$.

    %The first step in formulating the eigenvalue problem posed by
    %Eq.~\eqref{eq:DimensionlessEigenproblem} is to focus on a finite closed
    %interval $I=[0, t_\mathrm{tmax}]$, where $t_\mathrm{max}>0$ sets the maximal
    %radius that is considered. The value $t_\mathrm{max}$ must be chosen large
    %enough to ensure that the Dirichlet boundary condition
    %$f_{n,l,s,j}(t\geq{t}_\mathrm{max})=0$ is satisfied, i.e., that the support
    %of the desired eigenfunction lays within the interval $I$. For finite
    %$t_\mathrm{max}$ this is not possible exactly, because the radial
    %eigenstates asymptotically approach zero. However, numerically, the value
    %zero means less than machine precision such the boundary condition can
    %be satisfied numerically even for finite $I$.

    %The mesh we define as a family
    ...
    %canonical choice $\phi_0=\phi_{N-1}\equiv0$.
