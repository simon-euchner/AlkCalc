...
Testing.
    ...
    1. Generate the eigenenergies and radial eigenstates for the species 1H, as
       ...
       with offset = -nl + 1 = -l in interface/settings.c. The maximal principal
       quantum number must satisfy nmax >= 10, and rmax must be chosen large
       enough to support the state with n = nmax.
    ...
    2. Make sure that the mass correction is NOT included, that is, that the
       ...
       is the default behaviour of AlkCalc (see the section 'Important
       additional information'). The reference values of the tests are the exact
       analytical results of the Hydrogen problem.

    3. Run the Makefile in the directory AlkCalc/tests with the argument 'test'.
       This compiles tests.c, links it against AlkCalc's library, and runs all
       tests at once.
...
Important additional information.
    ...
    - In interface/settings.c the total angular momentum quantum number, j, is
