...
\begin{document}
    ...
    \begin{equation}
        ...
        =
        \sum^{j'}_{m'_j=-j'}
        \frac{2}{3} E_\mathrm{f,i}
        \abs*{\langle\bm{\Psi}_\mathrm{f},\bm{r}\bm{\Psi}_\mathrm{i}\rangle}^2
    \,,
    \end{equation}
    with the energy difference $E_\mathrm{f,i}=E_\mathrm{f}-E_\mathrm{i}$. Note
    that we sum over the final $m'_j$, because we are interested in transitions
    between the fine-structure states, without resolving the Zeeman levels. The
    oscillator strength is a measure for how strongly the induced Hertzian
    (electric) dipole oscillates: the radiated energy is large for spatially
    large dipoles (captured by the matrix element) and fast oscillation
    frequencies (captured by the energy difference). In principle, we already
    have all the machinery to numerically compute this quantity, however, one
    can reduce the expression to a much simpler form by using angular momentum
    algebra, in particular, relations from \cite{varshalovic1988}. To simplify
    the expression, we first use (Condon-Shortley phase)
    \begin{equation}
        r_x
        =
    ...
    \begin{equation}
        f_{\mathrm{i}\to\mathrm{f}}
        =
        \frac{4\pii}{3} \frac{2}{3} E_\mathrm{f,i}
        \langle{R}_\mathrm{f},r^2R_\mathrm{i}\rangle^2
        \sum^{j'}_{m'_j=-j'}
        \sum^1_{q=-1}
    ...
    \begin{equation}\label{eq:OscillatorStrengthSimplified}
        ...
        =
        \frac{2}{3} E_\mathrm{f,i}
        \abs*{\langle r R_{n',l',s,j'}, r^2 R_{n,l,s,j} \rangle}^2
        a_l
    \,, \
        a_l
        =
    ...
    a rate, that describes the number of spontaneous emissions per unit time.
    Therefore, the total decay rate, at zero temperature is given by the sum
    over all Einstein-A coefficients. In the dipole approximation this sum is
    given by the expression \cite{friedrich2017}:
    ...
    \,, \
        A_{\mathrm{i}\to\mathrm{f}}
        =
        \frac{4}{3} \alpha^2 E^3_\mathrm{i,f}
        \sum_{u=x,y,z}
        \abs*{\langle\bm{\Psi}_\mathrm{f},r_u\bm{\Psi}_\mathrm{i}\rangle}^2
    \,.
    \end{equation}
    In terms of the oscillator strength $f_{\mathrm{i}\to\mathrm{f}}$ this
    ...
    quantum numbers the sum is running over. This is the correct decay rate only
    in the zero temperature case. For finite temperature, spontaneous
    transitions increasing the energy can also be driven, for sufficiently
    strong thermal fluctuations. The background spectral density is given by
    Plank's law, which yields for the average photon number at frequency
    $\omega$ and temperature $T$:
    \begin{equation}
        n(\omega,T)
        =
    ...
    \begin{equation}
        ...
        +
        \sum_{n',l',j'}
        E^2_\mathrm{f,i} f_{\mathrm{i}\to\mathrm{f}}
        (1+n(E_\mathrm{i,f},T))
    \,,
    \end{equation}
    where each sum only runs over quantum numbers such that the associated
    term is positive. This means, the first contribution is only over final
    state with lower energy and the second one only over final states with
    larger energy the initial state. With this formula the lifetime of a
    fine-structure state can then be computed as $\tau=\Gamma^{-1}$.
    ...
    \begin{sdocfun}
    {
        ...
        does not, the returned values is wrong and no error is thrown.
    }
        \sdocarg{char *species}{
            String to specify atom/ion species, e.g., \texttt{1H} for Hydrogen,
            or \texttt{88SR+} for the $\mathrm{^{88}Sr^+}$ ion.
