...
\setparsizes{0pt}{0pt}{0pt plus 1fill}
...
%%% CUSTOM COMMANDS
\newcommand{\ind}{\hspace*{1.5em}} % MANUALLY SET INDENTATION
...
\begin{document}
    ...
    \ind Bound single-electron eigenfunctions of alkali-metal atoms and
    ...
    Simulation, pure Rydberg Physics, Quantum Optics, \dots). Generally, because
    ...
    \ind This document's purpose is to define the Hamiltonian which \alkcalc is
    ...
    \ind The starting point is an alkali-metal atom or alkaline-earth-metal ion.
    We are interested in systems where all electrons are in closed shells except
    for one (e.g., $\mathrm{^1H}$, $\mathrm{^{38}Rb}$, $\mathrm{^{88}Sr^+}$).
    This allows a Kepler-problem description for the `effective nucleus' (the
    nucleus `screened' by the closed-shell electrons) and valence electron. To
    be concrete, we study the Hamiltonian
    \begin{equation}
    ...
    ion's mass $M$. The centre-of-mass motion is decoupled, such that
    $\bm{r},\bm{p}$ are the relative canonical co-ordinates, i.e., $\bm{r}$ is
    the distance vector between the centre-of-mass position of the effective
    nucleus and the valence electron.  Moreover, $\bm{L}$, $\bm{S}$, and
    ...
    respectively. The potential $V$ is not a simple Coulomb potential, but a
    generalised potential which includes different contributions (these arise
    when one starts with the full many-body electron Hamiltonian and
    incorporates relativistic and quantum electrodynamics effects). Following
    Refs.~\cite{aymar1996,marinescu1994,wilkinson2025} we write:
    \begin{equation}
        V(\norm*{\bm{r}};\bm{L}^2,\bm{S}^2,\bm{J}^2)
        =
        V_\mathrm{C}(\bm{r})
    ...
    \end{equation}%
    %
    which accounts for the polarization of the screened nucleus due to the
    ...
    fine-structure basis states $Y_{l,m_l}\otimes\bm{\chi}_{s,m_s}$, employing
    ...
    chosen in a ways such that the blocks are already separated. In this case we
    ...
    Therefore, if we manage to obtain $R_{n,l,s}$ the full spectrum is known.
    ...
    \ind This is now the part where \alkcalc comes into play: the radial
    eigenstates cannot be obtained analytically. \alkcalc provides a platform to
    solve for these states numerically. To make the problem addressable by a
    computer, we express the eigenproblem in terms of dimensionless quantities.
    To this end we express the Hamiltonians $H_{l,s,j}$ in units of Hartree,
    $E_\mathrm{H}=1\slash{(4\pii\varepsilon_0a_\mathrm{B})}$, where
    $a_\mathrm{B}=\hbar\slash{(m_\mathrm{e}c\alpha)}\approx52.9\,\mathrm{pm}$
    ($m_\mathrm{e}$: electron mass) is Bohr's radius. The radial eigenfunctions
    we parametrise as
    ...
    \ind To solve the eigenproblem in Eq.~\eqref{eq:eigenproblem} numerically,
    we employ a \ac{fem} instead of the conceptually (and programmatically) more
    ...
    slowly for large ones, where the particle is almost free. Thus, a mesh
    tailored to this behaviour leads to a significant reduction of computational
    resources (the fast oscillation can be efficiently resolved without
    introducing `too many' points in the slowly oscillating regime).

    \ind The first step in setting up the \ac{fem} is to agree on a finite
    closed interval $I=[0, t_\mathrm{tmax}]$, where $t_\mathrm{max}>0$ sets the
    maximal value of the domain we consider. The value $t_\mathrm{max}$ must be
    chosen large enough to ensure that the boundary condition
    ...
    $B(\phi''_k,\phi_{k'})=-B(\phi'_k,\phi'_{k'})$.}
    ...
    \ind Next, we express the normalisation condition in
    ...
    \ind Finally, we compute radial matrix elements. To this end, we first
    calculate for $p\in\mathbb{R}$ such that the integrals exist,
    \begin{align}\label{eq:radialMatrixElement}
        \begin{split}
            \frac{r^{n,l,s,j;n',l',s',j'}_p}{\aB^p}
            &\equiv
            \frac{1}{\aB^p} \langle R_{n,l,s,j},
            \hat{r}^p R_{n',l',s',j'} \rangle
            =
    ...
    \end{equation}%
    %
    To numerically compute the remaining integrals we use a two-point Gaussian
    quadrature, implemented by the approximation \cite{burden2011}:
    \begin{equation}
    ...
    result is exact if $\alpha$ is a polynomial of degree three, i.e., when
    computing dipole matrix elements the result is exact. We start with the
    diagonal entries of $R_p$, which read
    \begin{equation}
        (R_p)_{k,k}
        =
        \int^{t_k}_{t_{k-1}} \dd{t} \phi^2_k(t) t^p
        +
        \int^{t_{k+1}}_{t_k} \dd{t} \phi^2_k(t) t^p
    \,.
    \end{equation}
    %
    This is straight-forward to implement. However, note that for too large $p$
    there can be overflow or underflow in computing $t^p$. However, for 64-bit
    floating point arithmetic, $p=\pm10$ is easily possible, such that all
    typically appearing practical cases are included. With this result it is
    straight-forward to construct the matrix $R_p$, that is, it is
    straight-forward to compute arbitrary radial matrix elements.

    \ind Note that the integrals can also be calculated analytically. This
    results in
    \begin{equation}
        B(\phi_k,\mathrm{id}^p\phi_k)
        =
        \begin{cases}
                \frac{1}{(t_k-t_{k-1})^2}
                \bigg[
                    \frac{t^2_k-t^2_{k-1}}{2}
                    -2t_{k-1}(t_k-t_{k-1})
                    +t^2_{k-1}\ln(\frac{t_k}{t_{k-1}})
                \bigg]
            \\
                +
                \frac{1}{(t_{k+1}-t_k)^2}
                \bigg[
                    \frac{t^2_{k+1}-t^2_k}{2}
                    -2t_{k+1}(t_{k+1}-t_k)
                    +t^2_{k+1}\ln(\frac{t_{k+1}}{t_k})
                \bigg],
            & p=-1
        \\
            \frac{1}{(t_k-t_{k-1})^2}
            \bigg[
                t_k-t_{k-1}
                -2t_{k-1}\ln(\frac{t_k}{t_{k-1}})
                +t^2_{k-1}\big(\frac{1}{t_k}-\frac{1}{t_{k-1}}\big)
            \bigg]
        \\
            +
            \frac{1}{(t_{k+1}-t_k)^2}
            \bigg[
                t_{k+1}-t_k
                -2t_{k+1}\ln(\frac{t_{k+1}}{t_k})
                +t^2_{k+1}\big(\frac{1}{t_{k+1}}-\frac{1}{t_k}\big)
            \bigg],
            & p=-2
        \\
            \frac{1}{(t_k-t_{k-1})^2}
            \bigg[
                \ln(\frac{t_k}{t_{k-1}})
                -2t_{k-1}\big(\frac{1}{t_k}-\frac{1}{t_{k-1}}\big)
                +t^2_{k-1}\big(\frac{1}{t^2_k}-\frac{1}{t^2_{k-1}}\big)
            \bigg]
        \\
            +
            \frac{1}{(t_{k+1}-t_k)^2}
            \bigg[
                \ln(\frac{t_{k+1}}{t_k})
                -2t_{k+1}\big(\frac{1}{t_{k+1}}-\frac{1}{t_k}\big)
                +t^2_{k+1}\big(\frac{1}{t^2_{k+1}}-\frac{1}{t^2_k}\big)
            \bigg],
            & p=-3
        \\
            \frac{1}{(t_k-t_{k-1})^2}
            \bigg[
                \frac{t^{p+3}_k-t^{p+3}_{k-1}}{p+3}
                -2t_{k-1}\frac{t^{p+2}_k-t^{p+2}_{k-1}}{p+2}
                +t^2_{k-1}\frac{t^{p+1}_k-t^{p+1}_{k-1}}{p+1}
            \bigg]
        \\
            +
            \frac{1}{(t_{k+1}-t_k)^2}
            \bigg[
                \frac{t^{p+3}_{k+1}-t^{p+3}_k}{p+3}
                -2t_{k+1}\frac{t^{p+2}_{k+1}-t^{p+2}_k}{p+2}
                +t^2_{k+1}\frac{t^{p+1}_{k+1}-t^{p+1}_k}{p+1}
            \bigg],
            & \text{else}
        \end{cases}
    \end{equation}%
    ...
    \begin{equation}
        ...
        =
        \begin{cases}
            \frac{t^2_{k+1}-2t_{k+1}\ln(\frac{t_{k+1}}{t_k})t_k-t^2_k}
                 {2(t_{k+1}-t_k)^2},
            & p=-1
        \\
            \frac{(t_{k+1}+t_k)\ln(\frac{t_{k+1}}{t_k})-2(t_{k+1}-t_k)}
                 {(t_{k+1}-t_k)^2},
            & p=-2
        \\
            \frac{\frac{t_{k+1}}{t_k}
                  \big(1-\frac{t^2_k}{t^2_{k+1}}\big)
                  -2\ln(\frac{t_{k+1}}{t_k})}
                 {2(t_{k+1}-t_k)^2},
            & p=-3
        \\
            \frac{1}{(t_{k+1}-t_k)^2}
            \bigg[
                \frac{t_{k+1}t^{p+2}_k-t_kt^{p+2}_{k+1}}
                     {(p+1)(p+2)}
                +
                \frac{t^{p+3}_{k+1}-t^{p+3}_k}{(p+2)(p+3)}
            \bigg],
            & \text{else}
        \end{cases}
    \,.
    \end{equation}%
    %
    \label{page:divergingIntegral}%
    We emphasise here that for $k=1$ and $p$ negative enough, the above
    expression is not well-defined. This does not mean the matrix element does
    not exist, because the functions $f_{n,l,s,j}$ and $f_{n',l',s',j'}$ tend to
    zero as $t\to0$. If the radial matrix element exists, must therefore be
    determined before computing it, e.g., by considering the form of the radial
    equation for $t\ll{1}$, and thereby inferring the functional behaviour of
    the radial eigenstates close to zero. When implementing the radial matrix
    elements in a computer program, one may allow arbitrary (at least as long as
    no numerical limits are reached) $p$ by simply setting $(R_p)_{1,1}=0$ (we
    will see below that the off-diagonal components are always well-defined).
    This simplifying `error' is justified by the fact that the step
    $h_1=t_1-t_0$ should be chosen small (${\sim}10^{-6}$) in order to obtain
    sensible results for the eigenenergies and eigenstates, as well as the fact
    that the eigenfunctions $f_{n,l,s,j}$ vanish at $t=0$ fast enough (if the
    integral exists mathematically). Hence, the error one makes will be
    negligible, because, from a physical standpoint, it is clear that the radial
    eigenstates do not change much on the length scale ${\sim}10^{-6}\aB$.
    ...
    \ind Reference manual for the \alkcalc library functions. Prototypes for the
    ...
    \begin{sdocfun}
    {
        ...
        \sdocret{alkcalc\tus{state} *f}{
            ...
            \texttt{double *h}: Array of length $N-1$, containing the family of
            step sizes, $(h_k)_{k=1,\dots,N-1}$.\par%
    ...
    \begin{sdocfun}
    {
        ...
        Eq.~\eqref{eq:radialMatrixElement}]. Note that for too large and too
        negative $p$ the result's numerical error increases due to the finite
        precision used to compute differences of large numbers (catastrophic
        cancellation). To check for validity, use $\mathrm{^1H}$ (Hydrogen atom)
        and $\mathrm{^2He^+}$ (Helium ion) as a benchmark for atoms and ions,
        respectively. It is the user's responsibility to ensure that the
        integral associated to the desired matrix element exists. In case is
        does not, the returned values is wrong and no error is thrown (see also
        the discussion on p.~\pageref{page:divergingIntegral}).
    }
        \sdocarg{char *species}{
            String to specify atom/ion species, e.g., \texttt{1H} for Hydrogen,
            or \texttt{88SR+} for the $\mathrm{^{88}Sr^+}$ ion.
    ...
    \begin{sdocfun}
    {
        ...
        $\psi_\mathrm{i}$ and a final state $\psi_\mathrm{f}$. The dimensionless
        oscillator strength is defined as
        \begin{equation}
            f_{\mathrm{i\to{f}}}
            =
            \frac{2}{3}
            \frac{\me\omega_\mathrm{if}}{\hbar}
            \norm*{\langle \psi_\mathrm{i}, \bm{r}\psi_\mathrm{f} \rangle}^2
        \,,
        \end{equation}
        with the angular transition frequency
        $\omega_\mathrm{if}=(E_\mathrm{f}-E_\mathrm{i})\slash\hbar$ and the
        fine-structure energies $E_\mathrm{i}$ and $E_\mathrm{f}$ of the initial
        and final state, respectively, which are computable with the provided
        function \texttt{alkcalc\tus{E}nlsj}.
    }
        \sdocarg{char *species}{
            String to specify atom/ion species, e.g., \texttt{1H} for Hydrogen,
            or \texttt{88SR+} for the $\mathrm{^{88}Sr^+}$ ion.
        ...
        \sdocarg{int32\tus{t} ni}{
            Principal quantum number of the initial state $\psi_\mathrm{i}$.
        }
        \sdocarg{int32\tus{t} li}{
            Orbital angular momentum quantum number $l=1,2,\dots,n-1$ of the
            initial state $\psi_\mathrm{i}$.
        }
        \sdocarg{double si}{
            Electron spin quantum number of the initial state $\psi_\mathrm{i}$.
            Not an argument here, since $s=1\slash{2}$ is always assumed.
        }
        \sdocarg{double ji}{
            Total angular momentum quantum number of the initial state
            $\psi_\mathrm{i}$., $j$. Here, the following convention is applied:
        ...
        \sdocarg{double mji}{
            Total magnetic quantum number of initial state, $m_j=-j,...,j$ (same
            numerical convention as for \texttt{ji} [see above] and
            $k=0,\pm1,\pm2,\dots$).
        }
        \sdocarg{int32\tus{t} nf}{
            Principal quantum number of the final state $\psi_\mathrm{f}$.
        }
        \sdocarg{int32\tus{t} lf}{
            Orbital angular momentum quantum number $l=1,2,\dots,n-1$ of the
            final state $\psi_\mathrm{f}$.
        }
        \sdocarg{double sf}{
            Electron spin quantum number of the final state $\psi_\mathrm{f}$.
            Not an argument here, since $s=1\slash{2}$ is always assumed.
        }
        \sdocarg{double jf}{
        ...
        \sdocarg{double mjf}{
            Total magnetic quantum number of final state, $m_j=-j,...,j$ (same
            numerical convention as for \texttt{jf} [see above] and
            $k=0,\pm1,\pm2,\dots$).
        }
